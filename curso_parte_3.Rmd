...
Para tornar a manipulação de dados mais eficiente, foi criado um pacote chamado de dplyr que possui uma série de ferramentas para filtrar, selecionar e sumarizar informações, como veremos a seguir. Iremos ver duas caracteristicas importantes deste pacote, os "verbos" e os joins.
...
Como vimos, o dplyr é um pacote essencial para manipulação de dados e uma das suas vantagens é que a forma como aplicamos suas funções seguem o seguinte padrão:

`funcao_dplyr(banco_de_dados, argumentos_especificos)`

Ou seja, o primeiro argumento de qualquer um dos verbos é o banco de dados, seguido de argumentos especificos de cada função, como por exemplo, qual variável criar ou filtrar.

### `filter()`

Quando estamos fazendo algumas analises, é comum encontrarmos observações que não nos interessam, como por exemplo, se estou fazendo uma análise sobre candidatos de São Paulo, espero que na minha base de dados tenham apenas candidatos percentences a esta unidade federativa. Para isso utilizamos o `filter()`, vejamos o exemplo abaixo:
...
Vamos aproveitar para filtar os casos em que os Prefeitos concorrram em agremiações diferente de partidos isolados e guardar esse dado em um objeto chamado `pref_sup_SP`.
...
Em muitas situações nós não trabalhamos com todas as variáveis do bando de dados, para removermos aquelas colunas que nós não iremos utilizar utilizamos a função `select()`, como por exemplo, vamos selecionar apenas o nome dos candidatos e o número deles.
...
No exemplo acima estamos retirando as variáveis NM_CANDIDATO e NR_CANDIDATO colocando um `-` na frente das variáveis.
...
Perceba que para criar essa variável utilizamos uma função chamada `ifelse()`. Basicamente está função é um if que pergunta: Se a IDADE_ATUAL for menor ou igual à 40, classificamos como "MENOR QUE 40", caso negativo, "MAIOR QUE 40".

Agora que criamos a variável "CATEGORIA_IDADE", quantos candidatos tem uma idade menor de 40? Para respondermos isso podemos utilizar a função `count()`
...
Muitas vezes precisamos combinar informações de tabelas diferentes para conseguir responder as nossas perguntas, como por exemplo, vejamos as tabelas abaixo:

```{r}
knitr::include_graphics("imgs/join.png")
```

Estas duas tabelas são referentes à votos recebidos por um determinado candidato e a doação relacionada com ele porém, elas são duas tabelas diferentes cuja única fonte de ligação entre elas é a variável "ID". Sendo assim, para conectá-las precisaremos fazer com que exista um match entre ID e aqui entram os diferentes tipos de joins.
