...
Pois bem para _instalar_ o tidyverse basta digitarmos `install.packages("tidyverse")` e a partir daí esperar a instalação de todos os pacotes (este processo pode demorar!). E assim como todos os pacotes instalados podemos habilitá-los utilizando `library(tidyverse)`. 
...
```{r}
options(scipen=999) # Rodem isso antes, para não termos problemas com notação científica
library(tidyverse)
...
Quando estamos fazendo algumas análises, é comum encontrarmos observações que não nos interessam, como por exemplo, se estou fazendo uma análise sobre candidatos de São Paulo, espero que na minha base de dados tenham apenas candidatos pertencentes a esta unidade federativa. Para isso utilizamos o `filter()`, vejamos o exemplo abaixo:

```{r}
filter(candidatos, SG_UF == "SP")
```

Lembra quando vimos operadores lógicos? Pois bem, eles são muito utilizados no `filter()`. Caso o nosso interesse seja não apenas filtrar os candidatos de SP, mas também os candidatos à prefeitura adicionamos o nome da variável, seguido do condicional
...
Vamos aproveitar para filtrar os casos em que os Prefeitos concorram em agremiações diferentes de partidos isolados e guardar esse dado em um objeto chamado `pref_sup_SP`.
...
- Alterar DT_NASCIMENTO_ANO para numérico
...
O `group_by()` e `summarise()` são funções que trabalham, na maioria das vezes, juntas. Eles servem para sumarizar os dados de acordo com grupos, ou seja, a contagem que havíamos feito com o `count()` pode ser reproduzida aqui da seguinte forma:
...
Para solucionar este problema os desenvolvedores do pacote incluíram um operador chamado de pipe (`%>%`). Basicamente ele permite que possamos construir códigos de forma linear, como podemos notar abaixo!
...
### `left_join()` e `right_join()`
  ...
  Nesse caso pensamos se queremos que a nossa tabela pós join tenha o número de linhas da tabela da esquerda (`left_join`) ou da tabela da direita (`right_join`), utilizando o mesmo padrão do exemplo anterior.
...
Vamos juntar tudo o que aprendemos e tirar alguma informação dos dados? Quantos votos em média vereadorxs recebem por município e gênero?
...
Esta função nos permite ver os tipos de dados que estamos lidando, o número de observações e variáveis. Se nós percebermos bem, existe uma variável que liga os dois dataframes, a variável SQ_CANDIDATO! Não se esqueçam dela, pois ela é o identificador das tabelas.
