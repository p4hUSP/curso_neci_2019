...
Nesta primeira parte apresentaremos os primeiros passos para começarmos a programar utilizando a linguagem R. O objetivo desta parte é que você se sinta familiarizado com a programação e começe a entender a automatização de tarefas que pode te ajudar no seu dia de trabalho.
    
    2. Importação de dados
    
Aqui começamos a entender como aplicar os conhecimentos anteriores às nossas áreas de atuação. Veremos como abrir banco de dados em diversos formatos e jeitos.
    
    3. Manipulação de dados
    
A última parte vai apresentar técnicas de manipulação de dados no R, algo que demanda boa parte do tempo em pesquisas e trabalhos com análise de dados.

<center> Vamos lá? </center>

## Parte 1 - overview

Explorar o ambiente R
...
## Primeiro, vamos comparar com os softwares que conhecemos

Abrir ddrevisited_data_v1.xls
Abrir ddrevisited_data_v1.aacdb
Abrir ddrevisited_data_v1.dta

Onde estão as variáveis? E as observações?

# O que é o R e o RStudio?

## Linguagem de programação R

O __R__ é uma linguagem de programação que vem crescendo bastante nos últimos anos. Apesar de inicialmente ter como propósito cálculos estatísticos, a linguagem é flexivel o bastante para ir além disso, permitindo lidar com diferentes tipos de dados, como por exemplo, textos, números, imagens e etc.
...
Para solucionar isto e adicionar outras funcionalidades que melhoram a experiência de quem utiliza a linguagem, foi desenvolvido uma IDE (Ambiente de desenvolvimento integrado) chamado _RStudio_.
...
É sempre preferível trabalhar com scripts do que com diretamente com no console. Dessa forma salvamos cada passo de modificação do banco de dados.

Quando trabalhamos com dados que não coletamos, o ideal é realizar todas as alterações documentadas no script mantendo o documento original intacto.

Isso é importante para a replicabilidade de nossos trabalhos. Queremos que nossos resultados sejam encontrados por todas as pesquisadoras encontrem os mesmos resultados.    

A documentação do código também é essencial a compreensão dos comandos. Por isso os comentários são super úteis e importantes. 

# Nossas primeiras linhas de código

Uma das funcionalidades mais básicas do __R__ é realização de operações matemáticas. Se estamos digitando os comandos em um script basta colocarmos o cursor do mouse na linha do código e apertar `Ctrl + Enter` e o resultado sairá no Console, assim como já havíamos mencionado.
...
As estruturas de dados são maneiras de organizar e guardar dados dentro da linguagem para que possamos trabalhar de forma mais eficiente. No nosso cotidiano utilizaremos três destas estruturas: vetores atômicos, listas e dataframes; divididos em tipos de estruturas homogêneas e heterogêneas. Mas o que isso significa? Basicamente diz respeito a quantidade de tipos de dados que ela pode receber, isto é, estruturas homogêneas só podem receber um tipo de dado, enquanto as estruturas heterogêneas podem receber mais de um tipo de dado.
...
Imaginem um vagão de trêm, consideraremos o vagão como um vetor no qual os valores, isto é, os dados são os nomes dos passageiros, que no caso são variáveis do tipo texto. Se pensarmos em outro vagão podemos pensar na idade dos passageiros.

Outro exemplo, talvez mais próximo da análise de dados seja o seguinte:

Queremos trabalhar com dados dos presidentes eleitos de 1945 até 1960, para isso podemos construir vetores que nos informem os nomes, a porcentagem de votos recebida, a quantidade de UF em que ele ganhou e se seu vice era do mesmo partido.
...
Dado que criamos objetos, podemos acessar os valores específicos a partir da posição dentro do vetor, como por exemplo, se quisermos acessar o primeiro elemento do `presidente_eleito`, isto é, "Dutra". Para isso basta colocarmos `[]` após o vetor e dentro dele a posição de interesse.
...
Agora que criamos nosso primeiro dataframe podemos pensar em mais diferenças em relação à manipulação de dados em outros softwares.
No R, não existe a possibilidade de manipulararmos os dados direto do dataframe. Por isso, usamos funções (que logo mais veremos com mais calma) para conhecer melhor o dataframe.
...
5. Utilizando a indexação, obtenha o número de votos e o partido dos candidatos Fernando Haddad e Ciro Gomes e armazene em vetores diferentes.
